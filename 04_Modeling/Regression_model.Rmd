...
df1 <- df1[!is.na(df1$numberoffloorsininsuredbuilding), ]
```
...
### Note: should scale numeric data before doing analysis. I exclude 4 columns of "date"

```{r}
df = as.data.frame(scale(df1[,c(3,6,7,12,17,18)])) # standardize all numerical variables

df <- cbind(df1[,c(1,2,4,5,8,9,13,16,20,21,22)],df,df1[,19]) # Add categorical columns  back in
...
pcr.model = pcr(totalinsurancepremiumofthepolicy ~., data=train, validation ="CV", na.action = na.exclude)
...
dev.copy(jpeg, filename="PCR_Plot.jpg");
dev.off ()
