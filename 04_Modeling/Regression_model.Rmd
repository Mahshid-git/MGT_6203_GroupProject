...
bag.model <- randomForest(totalinsurancepremiumofthepolicy ~., data = train, mtry = 5, importance = TRUE)
...
```{r}
PCA_4 <- cbind(df[,19], PCA$x[,1:5])
modelPCA <- lm(V1~.,data = as.data.frame(PCA_4)) # fit model
summary(modelPCA) # not a great adjusted R square value comparing to previous model with k-fold

cross_valPCA <- cv.lm(as.data.frame(PCA_4), modelPCA, m=5) # cross-validate 
R2_PCA<- 1 - attr(cross_valPCA,"ms")*nrow(df)/sum((df$totalinsurancepremiumofthepolicy - mean(df$totalinsurancepremiumofthepolicy))^2) # calculate R-squared
R2_PCA
```
