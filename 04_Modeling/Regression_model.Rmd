...
```{r}
tree.model <- rpart(totalinsurancepremiumofthepolicy_log ~., data = train, method = "anova")
summary(tree.model)

fancyRpartPlot(tree.model, cex = 0.33)		
...
```{r}
rf.model <- randomForest(totalinsurancepremiumofthepolicy_log ~., data = trainmtry = 3, importance = TRUE)
plot(rf.model)
