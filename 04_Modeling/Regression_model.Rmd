...
for (i in 1:15) {
  ...
  pc_data <- cbind(df[,16], pc)  # create data set
  ...
  R2_cross_val <- 1 - attr(cross_val,"ms")*nrow(df)/sum((df$totalinsurancepremiumofthepolicy - mean(df$totalinsurancepremiumofthepolicy))^2) # calculate R-squared
