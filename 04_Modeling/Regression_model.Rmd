...
pred.tree.test <- predict(treet.model, test)
...
pred.rf.test <- predict(rf.model, test)
...
ggplot(imp, aes(x = reorder(variable, X.IncMSE), y = X.IncMSE, color = reorder(variable, X.IncMSE))) +
  ...
  labs(title = "Variable Importance for Prediction of Toatl Insurance Premium") +
  coord_flip() +
  scale_color_manual(values = colorRampPalette(brewer.pal(1,"Red"))(10)) +
  theme_classic()
```
...
# PCA: consider if we can reduce dimension
...
```
# Looking at scree plot, it is observed that we can select 4PCs, 7PCs or 10PCs which can represents for 61%, 88% and 96% of the data 

```{r}
PCA_4 <- cbind(df[,22], PCA$x[,1:4])
...
size_PCA <- floor(0.7*nrow(PCA))
train_ind_PCA <- sample(seq_len(nrow(PCA)), size = size_PCA)

train_PCA <- PCA[train_ind_PCA, ]
test_PCA <- PCA[-train_ind_PCA, ]
#head(test)
```


# 2. Lasso with PCA_10 components


```{r}
xtrain = model.matrix(totalinsurancepremiumofthepolicy_log ~., train_PCA)[,-1]
ytrain = train_PCA$totalinsurancepremiumofthepolicy_log
xtest = model.matrix(totalinsurancepremiumofthepolicy_log ~., test_PCA)[,-1]
ytest = test_PCA$totalinsurancepremiumofthepolicy_log
```
...
lasso.model = glmnet(xtrain_PCA, ytrain, alpha=1, lambda=bestlam.lasso)
