...
```{r}
df = as.data.frame(scale(df1[,c(4,9,11,12,13,14)])) # standardize all numerical variables
#df = as.data.frame(scale(df1[,c(3,6,7,15,16)]))
df <- cbind(df1[,c(1,2,3,5,6,7,8,10,15,16,17)],df) # Add categorical columns  back in
View(df)
```
