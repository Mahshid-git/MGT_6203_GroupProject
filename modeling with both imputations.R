...
View(df1)
```{r}
df = as.data.frame(scale(df1[,c(2,3,4,6,7,9,10,11,12,13)])) # standardize all numerical variables
#df = as.data.frame(scale(df1[,c(3,6,7,15,16)]))
df <- cbind(df1[,c(1,5,8,14,15,16)],df) # Add categorical columns  back in
View(df)
```
