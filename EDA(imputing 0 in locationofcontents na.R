...
```{r}
fit1 = lm(totalinsurancepremiumofthepolicy ~.-longitude -originalconstructiondate -originalnbdate -primaryresidenceindicator -floodzone_highrisk -policytermindicator -postfirmconstructionindicator -mobilehomeortrailer -floodzone_modrisk -floodzone_undetermined -basementonly -morethan1floor, df_clean)
summary(fit1)
```
...
```{r}
df_final <- subset(df_clean, select = -c(longitude,originalconstructiondate,originalnbdate,primaryresidenceindicator,floodzone_highrisk,policytermindicator,postfirmconstructionindicator,mobilehomeortrailer,floodzone_modrisk,floodzone_undetermined,basementonly,morethan1floor))
View(df_final)
```
...
```{r}
mod_Step = lm(totalinsurancepremiumofthepolicy ~ censustract+elevatedbuildingindicator+construction+deductibleamountincontentscoverage+
