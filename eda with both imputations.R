...
```{r}
fit1 = lm(totalinsurancepremiumofthepolicy ~.-lowerflooronly -longitude -upperandlowerfloors -floodzone_highrisk -censustract -primaryresidenceindicator -policytermindicator -postfirmconstructionindicator -mobilehomeortrailer -floodzone_modrisk -floodzone_undetermined -basementonly, df_clean)
summary(fit1)
```
...
```{r}
df_final <- subset(df_clean, select = -c(censustract,primaryresidenceindicator,policytermindicator,postfirmconstructionindicator,mobilehomeortrailer,floodzone_modrisk,floodzone_undetermined,basementonly,morethan1floor,basementenclosurecrawlspacetype,longitude,lowerflooronly,upperandlowerfloors,floodzone_highrisk))
View(df_final)
```
...
df_num <- subset(df_final, select = -c( policyeffectivedate, policyterminationdate,originalconstructiondate,originalnbdate))
View(df_num)
```
str(df_num)
```{r}
...
```{r}
mod_Step = lm(totalinsurancepremiumofthepolicy ~ longitude+elevatedbuildingindicator+construction+deductibleamountincontentscoverage+
                +elevationdifference+crsdiscount+totalbuildinginsurancecoverage+deductibleamountinbuildingcoverage+policycount+totalcontentsinsurancecoverage+
                policycost+ lowerlevelcondo+ numberoffloorsininsuredbuilding+basementenclosurecrawlspacetype,
              data = scaledData)
summary(mod_Step)
```
