...
train_con <- trainControl(method = 'cv', number = 2) # using 5 folds cv
...
# From the Lasso Regression models, we can discard the variables with coeeficients near to zero, but look like we should kep all. # so the totalinsurancepremiumofthepolicy will have 17 variables and 2 date variables
...
```{r}
weights = chi.squared(totalinsurancepremiumofthepolicy ~ ., df_num)
...
f = as.simple.formula(subset, "totalinsurancepremiumofthepolicy")
print(f)
```
