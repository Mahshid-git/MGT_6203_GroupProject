...
```{r}
fit1 = lm(totalinsurancepremiumofthepolicy ~. -censustract -originalnbdate -policytermindicator -basementonly  -upperlevelcondo -floodzone_modrisk -floodzone_undetermined, df_clean)
summary(fit1)
```
...
```{r}
df_final <- subset(df_clean, select = -c(censustract,originalnbdate,policytermindicator,basementonly, upperlevelcondo,floodzone_modrisk,floodzone_undetermined))
View(df_final)
```
