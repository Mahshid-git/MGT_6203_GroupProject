...
```{r}
fit1 = lm(totalinsurancepremiumofthepolicy ~. -censustract -policytermindicator -basementonly -elevationdifference -mobilehomeortrailer  -floodzone_modrisk -floodzone_undetermined, df_clean)
summary(fit1)
```
...
```{r}
df_final <- subset(df_clean, select = -c(censustract, policytermindicator, basementonly, mobilehomeortrailer, floodzone_highrisk, floodzone_modrisk, floodzone_undetermined, policyterminationdate, not_condo, u_condo, policyeffectivedate, crsdiscount, elevationdifference))
...
```{r}
# Select only numerical columns for data analysist (leave data type in df_final_filtered)
# df_num includes 20 variables
...
file_path3 <- "../03_EDA/df_final_0impute.parquet"
write_parquet(df_final_filtered, file_path3)

# df_num includes only numeric variables for analysis
file_path4 <- "../03_EDA/df_num_0impute.parquet"
...
```{r}
file_path <- "../03_EDA/df_num_0impute.parquet"
df_num <- read_parquet(file_path, as_tibble = TRUE)

file_path <- "../03_EDA/df_final_0impute.parquet"
df <- read_parquet(file_path, as_tibble = TRUE)
...
```{r}
fit2 = lm(totalinsurancepremiumofthepolicy ~., df)
summary(fit2)
```
...
```{r}
scaledData = as.data.frame(scale(df_num[,c(3,4,6,7,13,14,15)])) # standardize all numerical variables
scaledData <- cbind(df_num[,c(1,2,5,8,9,10,11,12,17,18,19,20)],scaledData,df_num[,16]) # Add categorical columns  back in
...
# From the Lasso Regression models, we can discard the variables with coeeficients decreasing to zero, in this case we will remove variable "construction"

```{r}
df_num = subset(df_num,select=-c(construction)) 
#View(df_num)
```
