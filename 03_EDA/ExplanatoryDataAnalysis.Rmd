...
fit1= train(totalinsurancepremiumofthepolicy~., data=df, method="rpart", trControl = train_control)
...
```{r}
set.seed(100)
size <- fllor(0.7*nrow(df_num))
train_ind <- sample(seq_len(nrow(df_num)), size = size)

train <- df_num[train_ind, ]
test <- df_num[-train_ind, ]
```
