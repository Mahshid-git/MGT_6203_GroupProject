---
title: "Untitled"
author: "Mahshid JafarPour; Anh Tran; Warda Khan, Hossein Daneshvar"
date: "02/04/2022"
output:
...
```
```{r}
temp0 = subset(df_num, select=-c(reportedzipcode, basementenclosurecrawlspacetype_log))
fit3 = lm(totalinsurancepremiumofthepolicy_log ~., temp0)
summary(fit3)
```
...
```{r}
scaledData = as.data.frame(scale(temp0[,c(2,3,4,6,7,8,9,13)])) # standardize all numerical variables
scaledData <- cbind(temp0[,c(5,10,11,12,14,15,16,17,18,19,20,21,22)],scaledData,temp0[,1]) # Add categorical columns  back in
...
# From the Lasso Regression models, we can discard the variables with coefficients decreasing to zero, in this case we will remove variable "basementonly", "deductibleamountinbuildingcoverage_trans" and "longitude"
