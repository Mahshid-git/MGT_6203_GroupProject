...
```{r}
fit1 = lm(totalinsurancepremiumofthepolicy ~.-censustract -basementandabove -primaryresidenceindicator -policytermindicator -postfirmconstructionindicator -mobilehomeortrailer -floodzone_modrisk -floodzone_undetermined -basementonly -morethan1floor, df_clean)
summary(fit1)
```
...
```{r}
df_final <- subset(df_clean, select = -c(censustract,basementandabove,primaryresidenceindicator,policytermindicator,postfirmconstructionindicator,mobilehomeortrailer,floodzone_modrisk,floodzone_undetermined,basementonly,morethan1floor))
View(df_final)
```
